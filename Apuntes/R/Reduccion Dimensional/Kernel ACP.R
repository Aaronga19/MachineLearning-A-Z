...
trainingSet_PCA = as.data.frame(predict(kpca,trainingSet))
trainingSet_PCA$Purchased = trainingSet$Purchased
testingSet_PCA = as.data.frame(predict(kpca,testingSet))
testingSet_PCA$Purchased = testingSet$Purchased
...
prob_pred = predict(classifier, type = "response", newdata = testingSet_PCA[,-3])
y_pred = ifelse(prob_pred>0.5,1,0)

# Crear matriz de Confusión 

cm = table(testingSet_PCA[,3], y_pred)
