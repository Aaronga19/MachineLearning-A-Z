...
dataset = dataset[, 3:5] # Filtrado de columnas si es necesario
...
# Ajustar modelo de Bosque de Desición con el conjunto de entrenamiento 

library(randomForest)
classifier = randomForest(x = trainingSet[,-3],
                          y = trainingSet$Purchased,
                          ntree = 10)

# Prediccion de nuevos resultados con el conjunto de Testing
y_pred = predict(classifier, newdata = testingSet[,-3]) # type= "class", para que me de resultadoes en 0 o 1 
...
# Representación del árbol clasificador 
plot(classifier,main = "Clasificador")
text(classifier)