...
# Ajustar modelo de Árbol de desición con el conjunto de entrenamiento 
...
y_pred = predict(classifier, newdata = testingSet[,-3], 
                 type = "class") # Class para que me de resultadoes en 0 o 1 
...
plot(set[, -3],
     main = 'Árbol de Desición (Conjunto de Entrenamiento)',
...
y_grid = predict(classifier, newdata = grid_set, type = "class")
plot(set[, -3],
     main = 'Árbol de Desición (Conjunto de Testing)',
     xlab = 'Edad', ylab = 'Sueldo Estimado',
     xlim = range(X1), ylim = range(X2))
contour(X1, X2, matrix(as.numeric(y_grid), length(X1), length(X2)), add = TRUE)
points(grid_set, pch = '.', col = ifelse(y_grid == 1, 'springgreen3', 'tomato'))
