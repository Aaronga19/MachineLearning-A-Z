# -*- coding: utf-8 -*-
"""
Created on Tue Nov 24 11:52:38 2020
...
X1, X2 = np.meshgrid(np.arange(start = X_set[:,0].min()-1,stop=X_set[:,0].max()+1, step= 1),
                     np.arange(start = X_set[:,1].min()-1,stop=X_set[:,1].max()+1, step= 500))# Particiones de las gráficas
...
plt.title("Arbol de Desición (Training set)")
plt.xlabel("Age")
...
for i,j in enumerate (np.unique(y_set)):
    plt.scatter(X_set[y_set== j,0], X_set[y_set==j,1],
                c = ListedColormap(('red','green'))(i),label = j)
plt.title("Arbol de Desición (Testing set)")
plt.xlabel("Age")
plt.ylabel("Estimated Salary")
plt.legend()
plt.show()
